...
async function loadAll(){
  const keys=['profile','tasks','events','subjects','summaries','meds','exams','cards','cases','stage'];
  for(const k of keys){ const v=await Store.get(k); if(v!=null) S[k]=v; }
...
async function askClaude(userText,opt){
  ...
  catch(netErr){ throw new Error(aiReady().proxy?'Connexion au service IA impossible. Vérifie ta connexion Internet.':'Les fonctions IA nécessitent une clé API. Ouvre Réglages pour l\'ajouter.'); }
  if(res.status===401||res.status===403){ throw new Error('Clé API refusée. Vérifie ta clé dans Réglages.'); }
  if(!res.ok){ let t='';try{t=await res.text();}catch(e){} throw new Error('Le service IA a répondu '+res.status+(t?' — '+t.replace(/\s+/g,' ').slice(0,150):'')); }
...
async function doSummary(){
  ...
  }catch(e){$('#pptOut').innerHTML=emptyBox('alert','Le résumé a échoué',esc(e.message)+'. Réessaie dans un instant.');}
...
async function doMeds(){
  ...
  }catch(e){$('#medOut').innerHTML=emptyBox('alert','Génération échouée',esc(e.message));}
...
async function doCase(){
  ...
  }catch(e){$('#caseOut').innerHTML=emptyBox('alert','Création échouée',esc(e.message));}
...
async function doQuiz(){
  ...
  }catch(e){$('#quizOut').innerHTML=emptyBox('alert','Création échouée',esc(e.message));}
...
async function doServiceSheet(){
  ...
  }catch(e){$('#svcOut').innerHTML=emptyBox('alert','Génération échouée',esc(e.message));}
...
async function doSchema(){
  ...
  }catch(e){$('#schOut').innerHTML=emptyBox('alert','Génération échouée',esc(e.message));}
...
function exportData(){
  const dump={_app:'NursaFlow',_v:2,_date:new Date().toISOString()};['profile','tasks','events','subjects','summaries','meds','exams','cards','cases','stage'].forEach(k=>dump[k]=S[k]);
...
function importData(input){
  ...
  rd.onload=async()=>{
    try{
      ...
      const keys=['profile','tasks','events','subjects','summaries','meds','exams','cards','cases','stage'];
      if(!keys.some(k=>k in d))throw new Error('Fichier non reconnu');
      for(const k of keys){ if(k in d){ S[k]=d[k]; await Store.set(k,d[k]); } }
      toast('Sauvegarde restaurée','ok');setTimeout(()=>location.reload(),600);
    }catch(e){toast('Fichier de sauvegarde invalide','err');}
  };
  rd.readAsText(f);
}
async function resetData(){for(const k of ['profile','tasks','events','subjects','summaries','meds','exams','cards','cases','stage']){await Store.set(k,null);}location.reload();}
