...
/* Modal accessible (P0-4) : role="dialog" (posé dans le squelette), libellé
   depuis le titre, focus déplacé dedans à l'ouverture, piégé (Tab boucle)
   tant que le modal est ouvert, puis rendu à l'élément déclencheur. */
const FOCUSABLE='button:not([disabled]),[href],input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])';
let _modalPrevFocus=null;
function modal(html,wide){
  const m=$('#modal'); m.className='modal'+(wide?' wide':''); m.innerHTML=html;
  const h=m.querySelector('h1,h2,h3'); m.setAttribute('aria-label',h?h.textContent.trim():'Fenêtre de dialogue');
  if(!$('#overlay').classList.contains('show')) _modalPrevFocus=document.activeElement;
  $('#overlay').classList.add('show');
  (m.querySelector(FOCUSABLE)||m).focus();
}
function closeModal(){
  const ov=$('#overlay'); if(!ov.classList.contains('show')) return;
  ov.classList.remove('show');
  if(_modalPrevFocus&&document.contains(_modalPrevFocus)){ try{_modalPrevFocus.focus();}catch(e){} }
...
document.addEventListener('keydown',e=>{
  ...
  if(e.key==='Tab'&&$('#overlay').classList.contains('show')){
    const m=$('#modal'), els=[...m.querySelectorAll(FOCUSABLE)];
    if(!els.length){ e.preventDefault(); m.focus(); return; }
...
/* Activation clavier des cliquables non-<button> (rôles button/checkbox posés
   sur .check, .q-opt, .cal-ev, .fc, …) : Entrée ou Espace déclenche le clic. */
document.addEventListener('keydown',e=>{
...
function openMobileMenu(){
  let html=`<div class="modal-head"><h2>Navigation</h2><button class="btn icon ghost" aria-label="Fermer" onclick="closeModal()">${ic('x')}</button></div><div class="modal-body" style="padding:12px">`;
  VIEWS.forEach(v=>{
    ...
    html+=`<button class="nav-item ${v.id===current?'active':''}" ${v.id===current?'aria-current="page" ':''}style="margin-bottom:2px" onclick="closeModal();go('${v.id}')">${ic(v.icon)}<span>${v.label}</span>${badge}</button>`;
  });
  html+=`</div>`;
  modal(html);
}
...
function taskRow(t){const s=SUBJECTS[t.subject];return `<div class="li ${t.done?'done':''}">
  ...
  <div class="check ${t.done?'on':''}" role="checkbox" tabindex="0" aria-checked="${!!t.done}" aria-label="Marquer « ${esc(t.title)} » comme ${t.done?'à faire':'faite'}" onclick="toggleTask('${t.id}')">${ic('check')}</div>
  ...
  <button class="btn icon ghost" onclick="delTask('${t.id}')" title="Supprimer" aria-label="Supprimer la tâche ${esc(t.title)}">${ic('trash')}</button></div>`;}
function taskModal(){
  modal(`<div class="modal-head"><h2>Nouvelle tâche</h2><button class="btn icon ghost" aria-label="Fermer" onclick="closeModal()">${ic('x')}</button></div>
...
function renderCal(){
  ...
  <div class="view">
    <div class="row between mb wrap" style="gap:10px">
      <div class="row" style="gap:6px"><button class="btn icon" aria-label="Semaine précédente" onclick="weekOffset--;renderCal()">${ic('chevL')}</button><button class="btn sm" onclick="weekOffset=0;renderCal()">Cette semaine</button><button class="btn icon" aria-label="Semaine suivante" onclick="weekOffset++;renderCal()">${ic('chevR')}</button></div>
  ...
  for(let i=0;i<7;i++){const d=new Date(base);d.setDate(base.getDate()+i);const iso=new Date(d.getTime()-d.getTimezoneOffset()*6e4).toISOString().slice(0,10);
    ...
    html+=`<div class="cal-day ${iso===todayISO()?'today':''}"><div class="row between"><span class="cal-dname">${dn[i]}</span><span class="cal-dnum">${d.getDate()}</span></div>
      ${evs.map(e=>`<div class="cal-ev" role="button" tabindex="0" aria-label="Modifier l'événement ${esc(e.title)}" style="background:${EVTYPES[e.type].hex}" onclick="evModal('${e.id}')">${e.time?e.time+' ':''}${esc(e.title)}</div>`).join('')}
...
function evModal(id,presetDate){
  ...
  modal(`<div class="modal-head"><h2>${e?'Modifier':'Nouvel'} événement</h2><button class="btn icon ghost" aria-label="Fermer" onclick="closeModal()">${ic('x')}</button></div>
  <div class="modal-body">
...
function openSubject(id){
  ...
  modal(`<div class="modal-head"><div style="width:38px;height:38px;border-radius:11px;background:${s.hex}18;color:${s.hex};display:grid;place-items:center">${subjIcon(id)}</div><h2>${s.name}</h2><button class="btn icon ghost" aria-label="Fermer" onclick="closeModal()">${ic('x')}</button></div>
  <div class="modal-body">
...
function objList(id){const o=S.subjects[id].objectives;return o.length?o.map(x=>`
  <div class="li" style="padding:8px 4px"><div class="check ${x.done?'on':''}" role="checkbox" tabindex="0" aria-checked="${!!x.done}" aria-label="Objectif : ${esc(x.text)}" onclick="toggleObj('${id}','${x.id}')">${ic('check')}</div>
    <div style="flex:1" class="li-title ${x.done?'done':''}">${esc(x.text)}</div>
    <button class="btn icon ghost" aria-label="Supprimer l'objectif" onclick="delObj('${id}','${x.id}')">${ic('trash')}</button></div>`).join(''):`<p class="muted" style="font-size:13px">Aucun objectif pour l'instant. Ajoute-en pour suivre ta maîtrise.</p>`;}
  ...
  <div class="view">
    ...
    <div class="card pad mt"><div class="sec-title">${ic('cards','ic-sm')} Toutes les cartes</div>
      ${S.cards.length?S.cards.map(c=>`<div class="li"><span class="dot" style="background:${SUBJECTS[c.subject]?.hex||'#888'};width:10px;height:10px"></span>
        ...
        <button class="btn icon ghost" aria-label="Supprimer la carte" onclick="S.cards=S.cards.filter(x=>x.id!=='${c.id}');save('cards');renderCards();buildNav()">${ic('trash')}</button></div>`).join(''):`<p class="muted" style="font-size:13px">Aucune carte. Crées-en ou génère-les depuis un résumé PPT.</p>`}</div>
  </div>`;
}
function cardModal(){modal(`<div class="modal-head"><h2>Nouvelle flashcard</h2><button class="btn icon ghost" aria-label="Fermer" onclick="closeModal()">${ic('x')}</button></div>
...
function savedSummaries(){return S.summaries.length?S.summaries.map(s=>{const su=SUBJECTS[s.subject];return`
  <div class="li"><span style="color:${su?su.hex:'#888'}">${su?subjIcon(s.subject):ic('fileText')}</span>
    ...
    <button class="btn icon ghost" aria-label="Supprimer le résumé" onclick="S.summaries=S.summaries.filter(x=>x.id!=='${s.id}');save('summaries');renderPPT()">${ic('trash')}</button></div>`;}).join(''):`<p class="muted" style="font-size:13px">Aucun résumé sauvegardé.</p>`;}
...
function savedMeds(){return `<div class="sec-title">${ic('cards','ic-sm')} Fiches sauvegardées</div>`+S.meds.map(m=>`
  <div class="li"><span style="color:var(--violet)">${ic('pill')}</span><div style="flex:1"><div class="li-title">${esc(m.patho)}</div><div class="li-meta">${m.items.length} médicaments · ${SUBJECTS[m.subject]?.name||''} · ${m.date}</div></div>
    <button class="btn sm" onclick="viewMeds('${m.id}')">Ouvrir</button><button class="btn icon ghost" aria-label="Supprimer la fiche" onclick="S.meds=S.meds.filter(x=>x.id!=='${m.id}');save('meds');renderMeds()">${ic('trash')}</button></div>`).join('');}
...
function viewMeds(id){const m=S.meds.find(x=>x.id===id);modal(`<div class="modal-head"><h2>${ic('pill')} ${esc(m.patho)}</h2><button class="btn icon ghost" aria-label="Fermer" onclick="closeModal()">${ic('x')}</button></div><div class="modal-body">${m.items.map(medCardHTML).join('<div style="height:14px"></div>')}</div>`,true);}
...
function savedCases(){return `<div class="sec-title">${ic('cards','ic-sm')} Cas sauvegardés</div>`+S.cases.map(c=>`
  <div class="li"><span style="color:var(--indigo)">${ic('clipboard')}</span><div style="flex:1"><div class="li-title">${esc(c.titre)}</div><div class="li-meta">${(c.subjects||[]).map(s=>SUBJECTS[s]?.name).filter(Boolean).join(' · ')} · ${c.date}</div></div>
    <button class="btn sm" onclick="viewCase('${c.id}')">Ouvrir</button><button class="btn icon ghost" aria-label="Supprimer le cas" onclick="S.cases=S.cases.filter(x=>x.id!=='${c.id}');save('cases');renderCases()">${ic('trash')}</button></div>`).join('');}
...
function quizQ(qq,i){
  ...
  if(qq.type==='ordre'){
    ...
    return head+arr.map((it,pos)=>`<div class="ord-item"><span class="num">${pos+1}</span><span style="flex:1">${esc(it.t)}${quizRevealed?` <b style="color:${it.i===pos?'var(--green-d)':'var(--pink-d)'}">(rang correct : ${it.i+1})</b>`:''}</span>${quizRevealed?'':`<span class="ord-btns"><button class="btn icon sm" aria-label="Monter l'étape" onclick="ordMove(${i},${pos},-1)" ${pos===0?'disabled':''}>${ic('up','ic-sm')}</button><button class="btn icon sm" aria-label="Descendre l'étape" onclick="ordMove(${i},${pos},1)" ${pos===arr.length-1?'disabled':''}>${ic('down','ic-sm')}</button></span>`}</div>`).join('')
  ...
  const opts=(qq.options||[]).map((o,oi)=>{
    ...
    return `<div class="q-opt ${cls}" ${quizRevealed?'':'role="button" tabindex="0" '}onclick="${quizRevealed?'':(qq.type==='sata'?`toggleSata(${i},${oi})`:`pickQcm(${i},${oi})`)}"><span class="key">${'ABCDE'[oi]||'?'}</span><span>${esc(o)}</span></div>`;
  }).join('');
...
function studyPlan(){
  ...
  modal(`<div class="modal-head"><h2>${ic('target')} Plan d'étude personnalisé</h2><button class="btn icon ghost" aria-label="Fermer" onclick="closeModal()">${ic('x')}</button></div>
...
function renderStageBody(){
  ...
  if(stageTab==='skills'){
    b.innerHTML=`<div class="card pad"><div class="row between mb"><div class="sec-title" style="margin:0">${ic('check','ic-sm')} Checklist des gestes techniques</div><button class="btn sm" onclick="addSkill()">${ic('plus')} Geste</button></div>
      ${S.stage.skills.map(s=>`<div class="li"><div class="check ${s.done?'on':''}" role="checkbox" tabindex="0" aria-checked="${!!s.done}" aria-label="Technique : ${esc(s.label)}" onclick="toggleSkill('${s.id}')">${ic('check')}</div><div style="flex:1"><div class="li-title ${s.done?'done':''}" style="font-size:14px">${esc(s.label)}</div>${s.done&&s.date?`<div class="li-meta">Pratiqué le ${fmtDate(s.date)}</div>`:''}</div><button class="btn icon ghost" aria-label="Supprimer la technique" onclick="delSkill('${s.id}')">${ic('trash')}</button></div>`).join('')}</div>`;
    ...
    b.innerHTML=`<div class="card pad"><div class="row between mb"><div class="sec-title" style="margin:0">${ic('clipboard','ic-sm')} Carnet de raisonnement clinique</div><button class="btn sm primary" onclick="journalModal()">${ic('plus')} Entrée</button></div>
      ...
      ${S.stage.journal.length?S.stage.journal.map(j=>`<div class="q-card"><div class="row between"><b style="font-size:14px">${esc(j.service||'Stage')} · ${fmtDate(j.date)}</b><button class="btn icon ghost sm" aria-label="Supprimer l'entrée" onclick="S.stage.journal=S.stage.journal.filter(x=>x.id!=='${j.id}');save('stage');renderStage()">${ic('trash')}</button></div>
...
function journalModal(){modal(`<div class="modal-head"><h2>Nouvelle entrée de raisonnement</h2><button class="btn icon ghost" aria-label="Fermer" onclick="closeModal()">${ic('x')}</button></div>
...
function schemaNode(k,ni){const nd=currentSchema.cols[k].nodes[ni];const ct=currentSchema.cols[k].title;
  modal(`<div class="modal-head"><h2>${esc(nd.label)}</h2><button class="btn icon ghost" aria-label="Fermer" onclick="closeModal()">${ic('x')}</button></div><div class="modal-body"><div class="chip violet mb">${esc(ct)}</div><p style="font-size:14.5px;line-height:1.6">${esc(nd.detail||'')}</p></div>`);
}
/* ==================================================================
...
const friday={
  ...
  show(html,acts){
    ...
    host.innerHTML=`<div class="friday-bubble"><button class="bclose" aria-label="Fermer la bulle de Friday" onclick="friday.hide()">${ic('x','ic-sm')}</button>
